...
{
    public partial class Frm_Makine3 : Form
    {
        #region "DEĞİŞKENLER"
        ...
        private void Frm_Makine3_Load(object sender, EventArgs e)
        {
        ...
        private void Frm_Makine3_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        public void UretimBitti_Kontrol()
        {
            if (UretimBitti_Tagi.OkunanDeger.ToBool() == true)
            {
                ...
                if (uretimEmir_Set.IsSucces)
                {
                    ...
                    Proxy.GetUretimEmirSonucBekleyenlerResponse get_UretimEmirSonuclar = Webservis.Get_UretimEmirSonucBekleyenler(lbl_WebservisMesaj, 33);

                    if (get_UretimEmirSonuclar.IsSucces)
                    {
                        int Lux1_Sonuc = Helper.TagDegerOku(sanver_AnaSayfa1, 102).ToInt();
                        int Lux2_Sonuc = Helper.TagDegerOku(sanver_AnaSayfa1, 104).ToInt();
                        int Lux3_Sonuc = Helper.TagDegerOku(sanver_AnaSayfa1, 106).ToInt();
                        int Lux4_Sonuc = Helper.TagDegerOku(sanver_AnaSayfa1, 108).ToInt();
                        int Lux5_Sonuc = Helper.TagDegerOku(sanver_AnaSayfa1, 110).ToInt();
                        int Lux6_Sonuc = Helper.TagDegerOku(sanver_AnaSayfa1, 112).ToInt();

                        double Lux1_Deger = Helper.TagDegerOku(sanver_AnaSayfa1, 103).ToDouble() / 10.0;
                        double Lux2_Deger = Helper.TagDegerOku(sanver_AnaSayfa1, 105).ToDouble() / 10.0;
                        double Lux3_Deger = Helper.TagDegerOku(sanver_AnaSayfa1, 107).ToDouble() / 10.0;
                        double Lux4_Deger = Helper.TagDegerOku(sanver_AnaSayfa1, 109).ToDouble() / 10.0;
                        double Lux5_Deger = Helper.TagDegerOku(sanver_AnaSayfa1, 111).ToDouble() / 10.0;
                        double Lux6_Deger = Helper.TagDegerOku(sanver_AnaSayfa1, 113).ToDouble() / 10.0;


                        for (int i = 0; i < get_UretimEmirSonuclar.UretimSonuclari.Length; i++)
                        {
                            if (get_UretimEmirSonuclar.UretimSonuclari[i].Tag == "20")
                                get_UretimEmirSonuclar.UretimSonuclari[i].Sonuc = string.Format("{0};{1}", Lux1_Sonuc, Lux1_Deger);

                            if (get_UretimEmirSonuclar.UretimSonuclari[i].Tag == "21")
                                get_UretimEmirSonuclar.UretimSonuclari[i].Sonuc = string.Format("{0};{1}", Lux2_Sonuc, Lux2_Deger);

                            if (get_UretimEmirSonuclar.UretimSonuclari[i].Tag == "22")
                                get_UretimEmirSonuclar.UretimSonuclari[i].Sonuc = string.Format("{0};{1}", Lux3_Sonuc, Lux3_Deger);

                            if (get_UretimEmirSonuclar.UretimSonuclari[i].Tag == "23")
                                get_UretimEmirSonuclar.UretimSonuclari[i].Sonuc = string.Format("{0};{1}", Lux4_Sonuc, Lux4_Deger);

                            if (get_UretimEmirSonuclar.UretimSonuclari[i].Tag == "24")
                                get_UretimEmirSonuclar.UretimSonuclari[i].Sonuc = string.Format("{0};{1}", Lux5_Sonuc, Lux5_Deger);

                            if (get_UretimEmirSonuclar.UretimSonuclari[i].Tag == "25")
                                get_UretimEmirSonuclar.UretimSonuclari[i].Sonuc = string.Format("{0};{1}", Lux6_Sonuc, Lux6_Deger);
        ...
        {
            lbl_Baslik.Text = string.Format("                            OTOTRİM - OFFICE FINAL MAKİNESİ - {0}", DateTime.Now.ToString());
        ...
        public void IsEmri_Iste_Kontrol()
        {
            ...
            if (IsEmri_Iste_Tagi.OkunanDeger.ToBool() == true)
            {
                ...
                {
                    Model.Uretim_Hat3 urun = Veritabani.UretimEmir_TablodanOku_Hat3(Model.Steps.Step2, 2);

                    if (urun.ID != 0)
                    {
                        //Urun bulundu plc'ye ürün bilgilerini ve reçeteyi gönder
                        PLC_Haberlesme.Makine3_Step2_VeriGonder(sanver_AnaSayfa1, urun);
                    }
                    else
                    {
