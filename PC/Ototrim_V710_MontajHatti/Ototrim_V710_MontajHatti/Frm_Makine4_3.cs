...
{
    public partial class Frm_Makine4_3 : Form
    {
        #region "DEĞİŞKENLER"
        ...
        public Frm_Makine4_3()
        {
            InitializeComponent();
        }

        private void Frm_Makine4_3_Load(object sender, EventArgs e)
        {
        ...
        private void Frm_Makine4_3_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        public void UretimBitti_Kontrol()
        {
            if (UretimBitti_Tagi.OkunanDeger.ToBool() == true)
            {
                ...
                Proxy.Set_UretimEmirResponse uretimEmir_Set = Webservis.Set_UretimEmir(IsEmriNo_Tagi.OkunanDeger, Model.Steps.Step2.ToInt(), Proxy.UretimEmirIslemTipleri.BITIR, false, hataliMi, HataKodu_Tagi.OkunanDeger.ToInt(), lbl_WebservisMesaj);
                ...
                if (uretimEmir_Set.IsSucces)
                {
                    ...
                    Proxy.GetUretimEmirSonucBekleyenlerResponse get_UretimEmirSonuclar = Webservis.Get_UretimEmirSonucBekleyenler(lbl_WebservisMesaj, 22);

                    if(get_UretimEmirSonuclar.IsSucces)
                    {
                        for (int i = 0; i < get_UretimEmirSonuclar.UretimSonuclari.Length; i++)
                        {
                            get_UretimEmirSonuclar.UretimSonuclari[i].Sonuc = string.Format("{0};{1};{2}"
                                , Helper.TagDegerOku(sanver_AnaSayfa1, 102 + i)
                                , Helper.TagDegerOku(sanver_AnaSayfa1, 112 + i).ToDouble() / 100.0
                                , Helper.TagDegerOku(sanver_AnaSayfa1, 122 + i)
                                );
                        }
        ...
        public void IsEmri_Iste_Kontrol()
        {
            ...
            {
                Model.Uretim_Hat2 urun = Veritabani.UretimEmir_TablodanOku_Hat2(Model.Steps.Step2, 0);

                if (urun.ID != 0)
                {
                    //Urun bulundu plc'ye ürün bilgilerini ve reçeteyi gönder
                    PLC_Haberlesme.Makine4_Step2_VeriGonder(sanver_AnaSayfa1, urun);
                }
                else
                {
